...
`nameFormat` / `detailsFormat` / `stateFormat` には `{SONG}` `{ARTIST}` `{LABEL}` `{BPM}` が使用できます。
...
| `presence.nameFormat` | `name` のフォーマット | `{SONG}` |
| `presence.detailsFormat` | `details` のフォーマット | `🎵 {SONG}` |
| `presence.stateFormat` | `state` のフォーマット | `🎤 {ARTIST}` |
